...
(function(){
 ...
 window.abrirFichaPedido=async function(id){
  ...
  let base=null;if(o.base_stock_item_id){const r=await supabaseClient.from('base_stock_items').select('*').eq('id',o.base_stock_item_id).maybeSingle();base=r.data||null}
  ...
  <div class="card" style="margin-top:14px"><h3>📝 Notas internas</h3><textarea id="pdInternal" rows="4">${esc(val(o,'internal_notes'))}</textarea></div><div style="display:flex;gap:10px;margin-top:16px"><button class="primary" style="flex:1" onclick="guardarFichaPedido('${o.id}')">Guardar cambios</button><button class="secondary" onclick="closeDrawer()">Cerrar</button></div>`;openDrawer();
 };
 ...
 window.guardarFichaPedido=async function(id){
  ...
  if(old)Object.assign(old,patch);toast('Ficha del pedido guardada');closeDrawer();drawOrders();
 };
 ...
 const oldDraw=window.drawOrders;window.drawOrders=function(){
  ...
  document.querySelectorAll('#orderTable .card > div > .card, #orderTable > div > .card').forEach((card,i)=>{
   ...
   if(!card.querySelector('.aihxo-full-order')){const b=document.createElement('button');b.className='primary aihxo-full-order';b.style.cssText='margin-top:8px;width:100%';b.textContent='📋 Ficha completa';b.onclick=()=>abrirFichaPedido(o.id);card.appendChild(b)}
  });
 };
})();