...
(function(){
  ...
  async function cargarPrendas(producto){
    ...
    const {data,error}=await supabaseClient
      .from('garments')
      .select('id,manufacturer,model,sizes,colors,active')
      .order('manufacturer')
      .order('model');
    ...
    const refrescar=(usarGuardado=true)=>{
      ...
      const tallasDisponibles=valores(g.sizes);
      const coloresDisponibles=valores(g.colors);
      const tallasElegidas=usarGuardado&&selector.value===producto.garment_id&&valores(producto.allowed_sizes).length
        ? producto.allowed_sizes : tallasDisponibles;
      const coloresElegidos=usarGuardado&&selector.value===producto.garment_id&&valores(producto.allowed_colors).length
        ? producto.allowed_colors : coloresDisponibles;
  ...
  window.productForm=async function(id){
    ...
    $('#drawerBody').innerHTML=`
      ...
      <div class="muted" style="margin-bottom:18px;line-height:1.45;">
        Aquí guardamos el diseño o artículo comercial. La camiseta física, sus tallas, colores y stock se gestionan en <b>Prendas base</b>.
      </div>

      <form class="form" id="pfV2">
        <div class="formgrid">
          <div class="field">
            <label>SKU del diseño/artículo</label>
            <input name="sku" value="${esc(p.sku||'')}" required autocomplete="off">
          </div>
          <div class="field">
            <label>Categoría</label>
            <select name="category">
              <option value="Camiseta" ${String(p.category||'').includes('Camiseta')?'selected':''}>Camiseta</option>
              <option value="Bolso" ${String(p.category||'')==='Bolso'?'selected':''}>Bolso</option>
            </select>
          </div>
        </div>

        <div class="field">
          <label>Nombre del diseño / producto</label>
          <input name="model" value="${esc(p.model||'')}" required placeholder="Ej. Atlantic Culture">
        </div>

        <div class="field">
          <label>Prenda base utilizada</label>
          <select name="garment_id" id="pfGarment" required></select>
          <div class="muted" style="margin-top:6px;">
            Ejemplo: Mukua · Melbourne Kids. El stock real pertenece a esta prenda, no al diseño.
          </div>
        </div>

        <div class="field">
          <label>Tallas permitidas para este diseño</label>
          <div id="pfAllowedSizes" style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:8px;"></div>
          <div class="muted" style="margin-top:6px;">Por defecto se activan todas las tallas de la prenda base. Desmarca las que no quieras vender con este diseño.</div>
        </div>

        <div class="field">
          <label>Colores permitidos para este diseño</label>
          <div id="pfAllowedColors" style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:8px;"></div>
        </div>
        ...
        <div class="field">
          ...
          <input id="productImageV2" type="file" accept="image/*">
          <div style="margin-top:10px;">
            ${p.image_url?`<img src="${esc(p.image_url)}" alt="Foto del producto" style="width:100%;max-width:220px;border-radius:14px;display:block">`:'<div class="muted">Sin foto principal</div>'}
          </div>
        </div>
        ...
        <div class="formgrid">
          <div class="field">
            ...
            <input name="dtf_cost" type="number" step=".01" value="${Number(p.dtf_cost||0)}">
          </div>
          <div class="field">
            <label>Extras</label>
            <input name="extras_cost" type="number" step=".01" value="${Number(p.extras_cost||0)}">
          </div>
        </div>

        <div class="field">
          <label>Precio de venta</label>
          <input name="sale_price" type="number" step=".01" value="${Number(p.sale_price||0)}">
        </div>

        <div class="formgrid">
          <div class="field">
            <label>Oferta · 1 impresión</label>
            <input name="price_one_print" type="number" step=".01" value="${p.price_one_print??''}">
          </div>
          <div class="field">
            <label>Oferta · 2 impresiones</label>
            <input name="price_two_print" type="number" step=".01" value="${p.price_two_print??''}">
          </div>
        </div>

        <div style="padding:12px 14px;border-radius:12px;background:#f7f9fc;margin:8px 0 16px;line-height:1.45;">
          <b>Stock:</b> se controla desde la prenda base y sus variantes de talla/color. Ya no se crea stock separado para cada diseño.
        </div>
    ...
    $('#pfV2').onsubmit=async e=>{
      ...
      const duplicate=products.find(x=>
        String(x.sku||'').trim().toLowerCase()===sku.toLowerCase()&&String(x.id)!==String(id||'')
      );
      ...
      if(imageFile){
        ...
        const safeSku=sku.replace(/[^a-zA-Z0-9-_]/g,'-')||'producto';
      ...
      const payload={
        ...
        category:/diseno propio|diseño propio/i.test(categoriaOriginal)?categoriaOriginal:f.get('category'),
        commercial_visibility:f.get('commercial_visibility')||'normal',
        allowed_sizes:permitidas.allowed_sizes,
        allowed_colors:permitidas.allowed_colors,
        image_url:imageUrl,
        dtf_cost:+f.get('dtf_cost')||0,
        extras_cost:+f.get('extras_cost')||0,
        sale_price:+f.get('sale_price')||0,
        price_one_print:f.get('price_one_print')!==''?+f.get('price_one_print'):null,
        price_two_print:f.get('price_two_print')!==''?+f.get('price_two_print'):null
      };

      let r;
      if(id){
        r=await supabaseClient.from('products').update(payload).eq('id',id);
      }else{
        r=await supabaseClient.from('products').insert({
          ...payload,
          size:null,
          color:null,
          stock:0,
