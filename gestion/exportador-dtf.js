...
(function(){
      ...
      <div class="grid two">
        <div class="card">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button type="button" class="primary dtfGarment" data-garment="light">☀️ Camiseta clara</button>
            <button type="button" class="secondary dtfGarment" data-garment="dark">🌙 Camiseta oscura</button>
          </div>
        ...
        <div class="card">
          ...
          <div class="muted" style="margin-top:9px;font-size:12px">Salida a ${DPI} ppp objetivo. El nombre del archivo incluirá medida y resolución en píxeles.</div>
        </div>
      </div>
    </div>`;
    bind();
  ...
  function bind(){
    ...
    document.querySelectorAll('.dtfGarment').forEach(b=>b.onclick=()=>{
      document.querySelectorAll('.dtfGarment').forEach(x=>x.className='secondary dtfGarment');
      b.className='primary dtfGarment';document.querySelector('#dtfGarmentValue').value=b.dataset.garment;refresh();
    });
  ...
  function contrastStats(img){
    const max=400,scale=Math.min(1,max/Math.max(img.naturalWidth,img.naturalHeight));
    const c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.naturalWidth*scale));c.height=Math.max(1,Math.round(img.naturalHeight*scale));
    const x=c.getContext('2d',{willReadFrequently:true});x.drawImage(img,0,0,c.width,c.height);const d=x.getImageData(0,0,c.width,c.height).data;
    let sum=0,n=0,dark=0,light=0;
    for(let i=0;i<d.length;i+=4){if(d[i+3]<40)continue;const lum=(0.2126*d[i]+0.7152*d[i+1]+0.0722*d[i+2])/255;sum+=lum;n++;if(lum<.22)dark++;if(lum>.88)light++;}
    return n?{avg:sum/n,dark:dark/n,light:light/n}:{avg:.5,dark:0,light:0};
  }
  ...
  function refresh(){
    ...
    document.querySelector('#dtfChecks').innerHTML=`
      <div style="display:grid;gap:8px">
        <div style="padding:10px 12px;border-radius:10px;background:#f6f8fb"><b>Salida:</b> ${fmt(w,1)}×${fmt(h,1)} cm · ${tW}×${tH} px · ${DPI} ppp objetivo</div>
        <div style="padding:10px 12px;border-radius:10px;background:#f6f8fb">${okRes?'✅':midRes?'⚠️':'❌'} <b>Resolución efectiva del original:</b> ${Math.round(effective)} ppp ${okRes?'· adecuada':midRes?'· utilizable con precaución':'· insuficiente para esa medida'}</div>
        <div style="padding:10px 12px;border-radius:10px;background:#f6f8fb">${transparency?'✅ Fondo transparente detectado':'⚠️ No se detecta transparencia. Si ves un fondo blanco/negro, también aparecerá en el archivo final.'}</div>
        <div style="padding:10px 12px;border-radius:10px;background:#f6f8fb">${contrastMsg}</div>
      </div>`;
    document.querySelector('#dtfExport').disabled=!(w>0&&h>0);
  }
  ...
  async function exportPNG(){
    ...
    const outW=pxForCm(w),outH=pxForCm(h);const c=document.createElement('canvas');c.width=outW;c.height=outH;const x=c.getContext('2d');x.clearRect(0,0,outW,outH);x.imageSmoothingEnabled=true;x.imageSmoothingQuality='high';
    if(fit==='stretch')x.drawImage(state.img,0,0,outW,outH);
    else{
