...
(function(){
 ...
 window.rentabilidadView=async function(c){
  const [{data:ex},{data:stock}]=await Promise.all([supabaseClient.from('expenses').select('*'),supabaseClient.from('base_stock_items').select('quantity,unit_cost')]);
  ...
  c.innerHTML=`<div class="page"><div class="section"><div><h2>📈 Rentabilidad real</h2><div class="muted">Ventas, costes, gastos y margen de AIHXO</div></div></div><div class="grid kpis">${kpi('Ventas',money(sales),valid.length+' pedidos no cancelados')}${kpi('Coste producción',money(costs),known+'/'+valid.length+' con coste real')}${kpi('Margen bruto',money(gross),sales?((gross/sales)*100).toFixed(1)+'%':'')}${kpi('Gastos',money(expensesTotal),'gastos generales')}${kpi('Resultado',money(net),'después de gastos')}${kpi('Dinero en stock',money(stockValue),'coste del inventario')}</div><div class="card"><div class="section"><div><h2>🏆 Rentabilidad por pedido</h2><div class="muted">Ordenados por margen</div></div></div><div class="table-wrap"><table><thead><tr><th>Pedido</th><th>Cliente</th><th>Venta</th><th>Coste</th><th>Margen</th></tr></thead><tbody>${ranked.map(o=>`<tr><td><b>${esc(o.order_number||'')}</b></td><td>${esc(o.customer_name||'')}</td><td>${money(o.total)}</td><td>${money(o._cost)}</td><td><b>${money(o._margin)}</b></td></tr>`).join('')}</tbody></table></div></div>${known<valid.length?`<div class="card" style="margin-top:14px">⚠️ <b>${valid.length-known} pedidos todavía no tienen desglose de coste real.</b><div class="muted">Puedes completarlo desde Pedidos → Ficha completa. Mientras tanto se utiliza el coste histórico cuando existe.</div></div>`:''}</div>`;
 };
 window.comprasView=async function(c){
  const [{data:suppliers},{data:purchases,error}]=await Promise.all([supabaseClient.from('suppliers').select('*').order('name'),supabaseClient.from('purchases').select('*,suppliers(name)').order('purchase_date',{ascending:false})]);if(error){console.error(error);return}
  const ps=purchases||[], total=ps.reduce((a,p)=>a+N(p.amount),0), pending=ps.filter(p=>!['recibido','completado','pagado'].includes(String(p.status||'').toLowerCase()));
  c.innerHTML=`<div class="page"><div class="section"><div><h2>🛒 Compras y proveedores</h2><div class="muted">Pedidos de material y gasto con proveedores</div></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button id="recvCamera" type="button" class="secondary">📷 Recibir con cámara</button><button id="newPurchaseBtn" type="button" class="primary">＋ Nueva compra</button></div></div><div class="grid kpis">${kpi('Compras',ps.length,'registradas')}${kpi('Importe',money(total),'total histórico')}${kpi('Pendientes',pending.length,'por recibir/cerrar')}${kpi('Proveedores',(suppliers||[]).length,'registrados')}</div><div class="card"><div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Proveedor</th><th>Compra</th><th>Importe</th><th>Estado</th><th></th></tr></thead><tbody>${ps.map(p=>`<tr><td>${esc(p.purchase_date||'')}</td><td>${esc(p.suppliers?.name||'—')}</td><td><b>${esc(p.purchase_number||p.description||'Compra')}</b><br><span class="muted">${esc(p.description||'')}</span></td><td>${money(p.amount)}</td><td>${esc(p.status||'')}</td><td><button type="button" class="secondary recvPurchase" data-purchase-id="${p.id}">📷 Recibir</button></td></tr>`).join('')}</tbody></table></div></div></div>`;
  window._aihxoSuppliers=suppliers||[];
  const abrir=pid=>{
   if(typeof window.abrirRecepcionEtiquetas==='function') window.abrirRecepcionEtiquetas(pid||null);
   else { console.error('recepcion-etiquetas.js no está disponible'); toast('No se pudo abrir la cámara. Recarga Gestión e inténtalo de nuevo.'); }
  };
 ...
 window.nuevaCompra=function(){
  const ss=window._aihxoSuppliers||[];
  document.getElementById('purchaseModal')?.remove();
  const modal=document.createElement('div');
  modal.id='purchaseModal';
  modal.style.cssText='position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.45);display:flex;align-items:flex-end;justify-content:center;padding:0';
  modal.innerHTML=`<div style="background:#fff;color:#111;width:100%;max-width:680px;max-height:92vh;overflow:auto;border-radius:20px 20px 0 0;padding:20px;box-sizing:border-box"><div style="display:flex;justify-content:space-between;align-items:center;gap:12px"><div><h2 style="margin:0">🛒 Nueva compra</h2><div style="color:#667085;font-size:14px;margin-top:4px">Registra un pedido realizado a un proveedor</div></div><button id="purchaseClose" type="button" style="border:0;background:#eee;border-radius:12px;padding:10px 13px;font-size:18px">✕</button></div><form id="buyForm" class="form" style="margin-top:16px"><div class="field"><label>Proveedor</label><select name="supplier_id" required><option value="">Selecciona</option>${ss.map(s=>`<option value="${s.id}">${esc(s.name)}</option>`).join('')}</select></div><div class="field"><label>Número / referencia</label><input name="purchase_number"></div><div class="field"><label>Descripción</label><input name="description" required></div><div class="field"><label>Importe €</label><input name="amount" type="number" min="0" step="0.01" required></div><div class="field"><label>Fecha</label><input name="purchase_date" type="date" value="${new Date().toISOString().slice(0,10)}" required></div><div class="field"><label>Estado</label><select name="status"><option>Pedido</option><option>Pagado</option><option>Recibido</option></select></div><div class="field"><label>Notas</label><textarea name="notes"></textarea></div><button type="submit" class="primary" style="width:100%;margin-top:8px">Guardar compra</button></form></div>`;
  document.body.appendChild(modal);
  const cerrar=()=>modal.remove();
  modal.querySelector('#purchaseClose').addEventListener('click',cerrar);
  ...
  modal.querySelector('#buyForm').addEventListener('submit',async e=>{
   ...
   const f=new FormData(e.target),payload=Object.fromEntries(f.entries());payload.amount=N(payload.amount);
   const {error}=await supabaseClient.from('purchases').insert(payload);
   if(error){console.error(error);toast('No se pudo guardar la compra');if(btn){btn.disabled=false;btn.textContent='Guardar compra'}return}
   toast('Compra registrada');cerrar();comprasView($('#view'));
  });
 };
