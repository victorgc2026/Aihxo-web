...
function stock(c){const total=products.reduce((a,p)=>a+p.stock,0),value=products.reduce((a,p)=>a+p.stock*cost(p),0),low=products.filter(p=>p.stock<=3);c.innerHTML=`<div class="page"><div class="grid four">${kpi('Unidades',total,'stock')}${kpi('Valor coste',money(value),'inventario')}${kpi('Referencias',products.length,'productos')}${kpi('Bajo mínimo',low.length,'reposición')}</div><div class="card"><div class="table-wrap"><table><thead><tr><th>SKU</th><th>Producto</th><th>Talla</th><th>Color</th><th>Coste</th><th>Stock</th><th></th></tr></thead><tbody>${products.map(p=>`<tr><td>${p.sku}</td><td>${esc(p.model)}</td><td>${esc(p.size||'')}</td><td>${esc(p.color||'')}</td><td>${money(cost(p))}</td><td><b class="${p.stock<=3?'red':'green'}">${p.stock}</b></td><td><button class="secondary" onclick="addStock('${p.id}')">＋</button></td></tr>`).join('')}</tbody></table></div></div></div>`}
