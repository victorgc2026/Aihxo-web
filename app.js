...
function drawOrders(){const q=($('#oq')?.value||'').toLowerCase(),a=orders.filter(o=>[o.order_number,o.customer_name,o.product_name,o.status].join(' ').toLowerCase().includes(q));$('#orderTable').innerHTML=a.length?`<div class="table-wrap" style="margin-top:14px"><table><thead><tr><th>Foto</th><th>Pedido</th><th>Fecha</th><th>Cliente</th><th>Producto</th><th>Cant.</th><th>Total</th><th>Estado</th></tr></thead><tbody>${a.map(o=>`<tr><td>
  <img
  ...
  <img
    ...
    style="width:50px;height:50px;object-fit:cover;border-radius:8px;display:none"
  >
</td><tr><td>${esc(o.order_number)}</td><td>${esc(o.order_date)}</td><td>${esc(o.customer_name)}</td><td>${esc(o.product_name)}<div class="muted">${esc(o.size||'')} · ${esc(o.color||'')}</div></td><td>${o.quantity}</td><td>${money(o.total)}</td><td><select onchange="updateStatus('${o.id}',this.value)">${STATUS.map(s=>`<option ${s===o.status?'selected':''}>${s}</option>`).join('')}</select></td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">No hay pedidos.</div>'a.forEach(async o=>{
